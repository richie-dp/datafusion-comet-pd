...
object Utils {
  ...
  def fromArrowType(dt: ArrowType): DataType = dt match {
    ...
    // Handle unsigned integers - map to signed types with larger bit width
  ...
  def getFieldVector(valueVector: ValueVector, reason: String): FieldVector = {
    valueVector match {
      case v @ (_: BitVector | _: TinyIntVector | _: SmallIntVector | _: IntVector |
          ...
          _: FixedSizeListVector | _: MapVector | _: UInt1Vector | _: UInt2Vector |
          _: UInt4Vector | _: UInt8Vector) =>
